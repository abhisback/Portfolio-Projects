...
-- use the database retail_db
USE RETAIL_DB;

-- creating table sales_fact
...
-- droppingg table sales_facT
DROP TABLE SALES_FACT;

-- Alternative method  for primary key
CREATE TABLE sales_fact
(
  order_id INT,
  customer_id INT NOT NULL,
...
-- creating table for order_product_mapping
...
-- foreign key constraint for sales_fact and order_product_mapping
ALTER TABLE order_product_mapping
ADD CONSTRAINT order_id_fn
  FOREIGN KEY (order_id) REFERENCES sales_fact (order_id);
  
-- foreign key constraint for order_product_mapping and product_info
ALTER TABLE order_product_mapping
ADD CONSTRAINT product_id_fn
  FOREIGN KEY (product_id) REFERENCES product_info(product_id);

-- foreign key constraint for sales_fact and costomer_info
ALTER TABLE sales_fact
ADD CONSTRAINT customer_id_fn
  FOREIGN KEY (customer_id) REFERENCES customer_info(customer_id);
  
-- foreign key constraint for sales_fact and branch_details
ALTER TABLE sales_fact
ADD CONSTRAINT store_id_fn
  FOREIGN KEY (store_id) REFERENCES branch_details(store_id);  
...
('55d7','Coca-Cola (Bottle)','soft drinks-juice and soda',80,80); 
 ...
 -- inserting data into order_product_mapping
...
(1549,'3128',8)
;
-- drop column lastname from table branch_details
ALTER TABLE BRANCH_DETAILS DROP COLUMN LAST_NAME;

-- inserting data into branch_details table
...
-- inserting values into customer_info table
...
-- writing query to find the number of transactions involving male customers
SELECT COUNT(ORDER_ID)
FROM SALES_FACT
WHERE CUSTOMER_ID IN (
SELECT CUSTOMER_INFO.CUSTOMER_ID
FROM CUSTOMER_INFO
WHERE CUSTOMER_INFO.GENDER = 'MALE'
);

-- alternative code for above question using inner join
SELECT COUNT(order_id)
FROM sales_fact s
...
-- writing query to find the number of customers who placed an order from more than one store
SELECT
  COUNT(customer_id)
FROM
...
-- writing query to output the city with the highest number of orders along with the number of orders
...
-- writing query to find the area that recorded the highest sales in terms of sales amount, along with its sales amount
...
-- writing query find the state with the highest number of customers
SELECT
  state, cust_count
FROM
  (SELECT
